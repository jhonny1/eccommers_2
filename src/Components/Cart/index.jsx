...
const Cart = () => {
    ...
    const [factura,setFactura] = useState(cartItems.reduce((previous,current)=> previous + current?.Cantidad_disponible * current?.Precio, 0))
    ...
    useEffect(() => {
      ...
      //  alert("modificado")
       setFactura(cartItems.reduce((previous,current)=> previous + current.Cantidad_disponible * current.Precio, 0))
    }, [cartItems]);


    // let total = cartItems.reduce((previous,current)=> previous + current.Cantidad_disponible * current.Precio, 0);
    const editarcupon = ({target})=>{
      const {value} = target;
        ...
        <div className={styles.cartContainer}> 
            ...
            >
              ...
              />
                 ...
                 {!cartOpen && <div className={`1 ${styles.productsNumber}`}>{productsLength}</div>}
            ...
            {cartItems.length && cartOpen &&(
                   <div className={styles.cart}>    
                      ...
                      <h4  className ={styles.descuento}>cupon</h4>
                      
                <input className={styles.inputdescuento}
                  type='descuento'
                  name='descuento'
                  id='descuento'
                  placeholder='descuento' 
                  value={cupon}
                  onChange={(e)=>editarcupon(e)}
                />
                    ...
                    <button onClick={() => aplicarDescuento()}>
                      ...
                      {cartItems.length ===  0 ? (
                      <p className={styles.cartVacio}></p>
                      ): (
                      ...
                      <h2 className={styles.total}>Total: ${factura}</h2>
                   </div>
            )}
        </div>
    );
